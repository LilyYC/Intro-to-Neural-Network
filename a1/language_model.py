...
DEFAULT_TRAINING_CONFIG = {'batch_size': 100,
                           ...
                           # the learning rate
                           'momentum': 0.9,
                           # the decay parameter for the momentum vector
                           'epochs': 50,
                           # the maximum number of epochs to run
                           'init_wt': 0.01,
                           # the standard deviation of the initial random weights
                           'context_len': 3,
                           # the number of context words used
                           'show_training_CE_after': 100,
                           # measure training error after this many mini-batches
                           'show_validation_CE_after': 1000,
                           # measure validation error after this many mini-batches
                           }
...
class Model(object):
    ...
    def __init__(self, params, vocab):
        ...
        self.embedding_dim = self.params.word_embedding_weights.shape[1] #16
        self.embedding_layer_dim = self.params.embed_to_hid_weights.shape[1] # 48
    ...
    def indicator_matrix(self, targets):
        """Construct a matrix where the kth entry of row i is 1
        if the target for example i is k, and all other entries are 0."""
    ...
    def compute_loss_derivative(self, output_activations, expanded_target_batch):
        ...
        #output_activations is 100 x 250, where 100 is the batch size, 250 is the number of vocab
        #expanded_target_batch: the target for the ith training case: 100 x 250 as well.
...
if __name__ == '__main__':
    ...
    find_occurrences('here', 'is', 'the')
    model.predict_next_word('here', 'is', 'the', k=10)
    model.tsne_plot()
