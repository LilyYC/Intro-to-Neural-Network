...
class MyDilatedConv2d(MyConv2d):
    ...
    def forward(self, input):
        new_kernel_size = self.kernel_size + \
                          self.dilation * (self.kernel_size - 1)
        weight = torch.zeros(
            self.out_channels, self.in_channels, new_kernel_size,
            new_kernel_size)
        if self.weight.data.shape[2] == self.kernel_size:
            for i in range(self.kernel_size):
                for j in range(self.kernel_size):
                    weight[:, :, i * (self.dilation + 1),
                    j * (self.dilation + 1)] \
                        = self.weight.data[:, :, i, j]
            self.padding = new_kernel_size // 2
        else:
            for i in range(self.kernel_size):
                for j in range(self.kernel_size):
                    weight[:, :, i * (self.dilation + 1),
                    j * (self.dilation + 1)] \
                        = self.weight.data[:, :, i * (self.dilation + 1),
                          j * (self.dilation + 1)]
...
class CNN(nn.Module):
    def __init__(self, kernel, num_filters, num_colours):
        ...
        self.upconv2 = nn.Sequential(
            MyConv2d(num_filters, num_colours, kernel_size=kernel,
                     padding=padding),
            nn.Upsample(scale_factor=2),
...
class UNet(nn.Module):
    def __init__(self, kernel, num_filters, num_colours):
        ...
        # Fifth Layer
        self.upconv2 = nn.Sequential(
            MyConv2d(num_filters * 2, num_colours, kernel_size=kernel,
                     padding=padding),
            nn.Upsample(scale_factor=2),
            nn.BatchNorm2d(num_colours),
            nn.ReLU())

        self.finalconv = MyConv2d(num_colours+1, num_colours,
                                  kernel_size=kernel)
...
class DilatedUNet(UNet):
    def __init__(self, kernel, num_filters, num_colours):
        ...
        self.rfconv = nn.Sequential(
            MyDilatedConv2d(num_filters * 2, num_filters * 2,
                            kernel_size=kernel, dilation=1),
            nn.BatchNorm2d(num_filters*2),
            nn.ReLU())
######################################################################
# Torch Helper
